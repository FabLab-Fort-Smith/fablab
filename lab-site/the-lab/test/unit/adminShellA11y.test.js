...
// Mock Next primitives so the real Sidebar renders in jsdom. next/image: pass only DOM-safe props
// (drop next-only props like `fill`).
jest.mock("next/navigation", () => ({ __esModule: true, usePathname: () => "/dashboard/admin" }));
jest.mock("next/link", () => ({ __esModule: true, default: ({ href, children, onClick, className, style, ...aria }) => <a href={href} onClick={onClick} className={className} style={style} {...aria}>{children}</a> }));
jest.mock("next/image", () => ({ __esModule: true, default: ({ alt = "", src, className, style }) => <img alt={alt} src={typeof src === "string" ? src : ""} className={className} style={style} /> }));
...
// The merged Sidebar (#119) fetches the dynamic addon nav on mount — stub it (empty plugin list).
beforeEach(() => {
  global.fetch = jest.fn(() => Promise.resolve({ ok: true, json: () => Promise.resolve({ plugins: [] }) }));
});
...
test("admin Sidebar has a named nav landmark, marks the active link, and is axe-clean", async () => {
  ...
  // Named navigation landmark (WCAG 1.3.1 / 2.4.1) — wait for the mount fetch to settle first.
  await waitFor(() => expect(getByRole("navigation", { name: /primary/i })).toBeInTheDocument());
