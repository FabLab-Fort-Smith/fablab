...
4. **S4 — Edge firmware (`pi-zero` edge role).** Sub-sliced during build.
   ...
   - **S4b** (next): the edge **runtime** — NFC read + strike-relay GPIO + mTLS client to the broker VIP +
     the local envelope store (persist + monotonic high-water) + **hardware RTC + monotonic clock floor**
     source (feeds S4a's `time_synced`/`now_ms`) + fail-secure supervisor (reconnect/backoff, heartbeat,
     `WatchdogSec`, OTA poll/commit) + **hash-chained store-and-forward audit** (`bootEpoch`). Bench one edge.
