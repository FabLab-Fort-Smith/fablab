    ...
    return (
        <div style={{ padding: '20px 24px' }}>
            ...
            <Section title="// INTEGRATIONS">
                {[
                    { name: 'Discord', desc: 'Link your Discord account to access the FabLab bot and community features.', connected: !!user.discordId, handle: user.discordHandle, onConnect: async () => { let marked = false; try { marked = (await fetch('/api/v1/auth/link-intent', { method: 'POST' })).ok; } catch { marked = false; } if (!marked) { showToast('Could not start Discord linking. Please try again.', 'error'); return; } signIn('discord', { callbackUrl: `/dashboard/${user.userID}/profile?tab=3` }); }, onDisconnect: async () => { if (!confirm('Disconnect your Discord account?')) return; const res = await fetch(`/api/v1/users?userID=${user.userID}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ discordId: '', discordHandle: '', discordLinked: false }) }); if (res.ok) { showToast('Discord disconnected.', 'success'); setTimeout(() => window.location.reload(), 1200); } else showToast('Failed to disconnect Discord.', 'error'); } },
