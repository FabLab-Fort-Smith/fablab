...
check_db_identity(){
  ...
  [ "$got_user" = "$(db_expected_user "$ENV_TARGET")" ] || mismatches+=("MONGODB_URI user: expected '$(db_expected_user "$ENV_TARGET")' got '$got_user'")
  [ "$got_db"   = "$exp" ] || mismatches+=("MONGODB_URI db path: expected '$exp' got '$got_db'")
  [ "$got_as"   = "$exp" ] || mismatches+=("MONGODB_URI authSource: expected '$exp' got '$got_as'")
  [ "$name"     = "$exp" ] || mismatches+=("MONGODB_NAME: expected '$exp' got '$name'")
  ...
  Fix MONGODB_URI + MONGODB_NAME in $ENVF to the $ENV_TARGET identity (password from
  MONGO_APP_PASSWORD_${ENV_TARGET^^}, generated by 'make secrets'), then re-run.
